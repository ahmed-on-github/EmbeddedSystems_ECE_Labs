...
int main(void){
    ...
    while (1){
		if( button_read(BUTTON_PORT,BUTTON1)== (uint8_t)1 ){
			...
			_delay_ms(1000); /*Delay allows for reading a press on BUTTON2*/
		}
		/*else{
		...
		if( button_read(BUTTON_PORT,BUTTON2)==(uint8_t)1 ){
			...
			_delay_ms(1000); /*Delay allows for reading a press on BUTTON3*/
		}
		/*else{
		...
		if( button_read(BUTTON_PORT,BUTTON3)==(uint8_t)1 ){
			...
			//_delay_ms(1000); /*no need for a delay here*/
		}
		/*else{
		...
		if( sum > 0 ){
            ...
            _delay_ms(500); /*Small delay after turn offs*/
		}	
    }
	return (uint8_t)0;
}
